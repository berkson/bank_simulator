...
@Slf4j
public class DebitStrategy implements OperationStrategy {
    ...
    public DebitStrategy(OperationDto operation) {
    ...
    public void execute() throws RequestProblemException {
        try {
            ...
            account.setBalance(account.getBalance().subtract(operation.getValue()));
            accountService.save(account);
            operationService.save(operation);
        } catch (Exception e) {
            log.error("Erro ao realizar débito. Erro {}", e.getMessage(), e);
            throw new RequestProblemException("Não foi possível realizar o débito", e);
        }
    }
}
